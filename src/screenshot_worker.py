...
class ScreenshotWorker:
    ...
    def __init__(self, base_url: str = "http://localhost:3000"):
        ...
        if self.supabase_url and self.supabase_key:
            ...
            logger.warning(
                "Supabase credentials missing. Screenshots will remain local only."
            )
    ...
    def capture_screenshot(
        self, url: str, output_path: str, mobile: bool = False
    ) -> bool:
        """
    ...
    def upload_to_supabase(self, local_path: str, storage_path: str) -> str:
        ...
        try:
            ...
            # Get public URL
            res = self.supabase.storage.from_("screenshots").get_public_url(
                storage_path
            )
            return res
        except Exception as e:
    ...
    def process_lead_list(self, leads: list, output_dir: str, file_prefix: str):
        ...
        for i, lead in enumerate(leads):
            name_clean = "".join(
                [c if c.isalnum() else "_" for c in str(lead.get("Name", "Unknown"))]
            )
...
if __name__ == "__main__":
    ...
    worker.capture_screenshot(
        worker.generate_url(test_lead), "exports/screenshots/test_business.png"
    )
