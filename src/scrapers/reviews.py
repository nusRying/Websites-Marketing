...
class ReviewsScraper(BaseScraper):
    ...
    async def get_reviews(self, page: Any, max_reviews: int = 50) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            while len(reviews) < max_reviews and retries < 5:
                ...
                more_btns = await page.query_selector_all(MAPS_SELECTORS["review_more_btn"])
                ...
                current_height = await page.evaluate(f"document.querySelector('{scroller_sel}').scrollHeight")
    ...
    async def _parse_reviews(self, page: Any) -> List[Dict[str, Any]]:
        ...
        for container in containers:
            try:
                ...
                reviewer_el = await container.query_selector(MAPS_SELECTORS["review_reviewer"])
                rating_el = await container.query_selector(MAPS_SELECTORS["review_rating"])
                ...
                rating_text = await rating_el.get_attribute("aria-label") if rating_el else "0 stars"
