...
class GoogleMapsScraper:
    ...
    def search(self, query: str, location: str, max_results: int = 20) -> List[Dict]:
        ...
        search_url = (
            f"https://www.google.com/maps/search/{urllib.parse.quote(full_query)}"
        )
        ...
        for link in all_links:
            ...
            if containers:
                ...
                for sub_link in sub_links:
                    ...
                    if "website" in sub_aria or sub_data == "website":
                        if not any(
                            domain in sub_href for domain in social_media_domains
                        ):
                            has_website = True
                            break
                    ...
                    if (
                        sub_href
                        and "google.com/maps" not in sub_href
                        and sub_href != url
                        and not sub_href.startswith("/")
                    ):
                        if not any(
                            domain in sub_href for domain in social_media_domains
                        ):
            ...
            leads.append({"name": name, "url": url, "has_website": has_website})
