...
class LeadExportHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if (
            ...
            and "_AI_ENRICHED_" not in filename
            and "_MASTER" in filename
        ):
    ...
    def trigger_enrichment(self, file_path):
        ...
        send_lead_alert(
            f"Pipeline Action: {filename}", "System Backend", "http://localhost:3000"
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"AI Enrichment failed with return code {result.returncode}"
                )
