...
def run_verification():
    ...
    if not no_website_leads:
        print(
            "No leads without websites found. Using a few from total for detail extraction test."
        )
        test_leads = leads[:2]
    else:
    ...
    for lead in test_leads:
        ...
        if website:
            is_social = any(
                domain in website.lower() for domain in social_media_domains
            )
            if not is_social:
        ...
        print(
            f"  -> KEEPING: No real website found (Website field: {website or 'None'})"
        )
