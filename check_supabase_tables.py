...
def check_tables():
    ...
    for table in tables:
        try:
            ...
            if "relation" in error_msg and "does not exist" in error_msg:
                ...
                print(
                    f"⚠️  Table '{table}': Created, but RLS is blocking access (Expected for Anon key)."
                )
    ...
    if all(v != "MISSING" for v in results.values()):
        ...
        print(
            "Some tables are missing. Please run the migration SQL in your Supabase SQL Editor."
        )
