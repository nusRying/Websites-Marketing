...
def test_pipeline():
            ...
            "reviews": [
                {
                    "author": "John",
                    "rating": 5,
                    "text": "Great service, very thorough cleaning!",
                }
            ],
        }
    ]
    ...
    enrich_cmd = [sys.executable, "src/ai_enrichment.py", "--input", raw_export_path]
    ...
    if os.path.exists(target_ss_dir):
        ...
        if screenshots:
            print(
                f"✅ Screenshots generated in: {target_ss_dir} ({len(screenshots)} files)"
            )
        else:
            print(
                f"⚠️  No screenshots found in {target_ss_dir} (Next.js server might not be running)"
            )
    else:
    ...
    print("Summary:")
    print("- Raw Export -> Enriched Export: PASS")
    print("- Column Mapping: PASS")
    print("- Logic Flow: PASS")
    print(
