...
def test_scraper_engine_retries_on_failure():
    ...
    mock_fetcher.fetch.side_effect = [Exception("Network error"), Exception("Network error"), "Success Response"]
...
def test_ai_enrichment_handles_malformed_json():
    ...
    try:
        ...
        if not hasattr(engine, 'generate_copy'):
             pytest.fail("AIEnrichmentEngine.generate_copy is missing from implementation!")
